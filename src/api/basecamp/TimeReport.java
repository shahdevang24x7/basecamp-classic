...
public class TimeReport extends BaseCampEntity {
	 ...
	 *            BCAuth Object
	 * @param spec
	 *            Defined Specifications for Report
	 */
	public TimeReport(BCAuth auth, TimeReportSpec spec) {
		...
		if (spec.subject_id != 0) {
			options += "subject_id=" + spec.subject_id + "&";
		}

		if (spec.from != null) {
			options += "from=" + sdf.format(spec.from.getTime()) + "&";
		}

		if (spec.to != null) {
			options += "to=" + sdf.format(spec.to.getTime()) + "&";
		}

		if (spec.filterCompanyId != 0) {
			options += "filter_company_id=" + spec.filterCompanyId + "&";
		}

		if (spec.filterProjectId != 0) {
			options += "filter_project_id=" + spec.filterProjectId + "&";
		}
	...
	public List<TimeEntry> getRangeOfEntries(Calendar start, Calendar end) {
		...
		for (int i = 0; i < this.entries.size(); i++) {
			...
			if ((start.equals(entryCal) || start.before(entryCal)) && (end.equals(entryCal) || end.after(entryCal))) {
